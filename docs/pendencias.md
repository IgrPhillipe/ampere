...
| 19 | Sem Flyway, `ddl-auto=update` não altera tipo de coluna já criada. A troca de `LocalDateTime` por `OffsetDateTime` em `Project` exige `docker compose down -v` em quem já tinha o volume. Reforça a pendência 16 | [`../app/back/src/main/java/br/com/ampere/domain/Project.java`](../app/back/src/main/java/br/com/ampere/domain/Project.java) | A definir | Aberta |
| 20 | Colunas "UCs" e "Demanda" e a busca por UC saíram da listagem: não existe entidade de unidade consumidora, e as duas colunas mostravam "—" em toda linha enquanto o campo de busca prometia procurar por UC. Voltam na US em que o dado existir | [`../app/front/src/features/projects/components/ProjectsTable/project-columns.tsx`](../app/front/src/features/projects/components/ProjectsTable/project-columns.tsx) | A definir | Aberta |
