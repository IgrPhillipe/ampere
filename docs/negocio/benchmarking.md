...
Análise de mercado para o desafio da Neoenergia Pernambuco: tratamento do cálculo de demanda em edificações com múltiplas unidades consumidoras por outras concessionárias e pela ferramenta do projetista.
...
Projetos elétricos de edificações com **múltiplas unidades consumidoras** apresentam alto índice de erro no cálculo de demanda. A complexidade dos critérios (muitos parâmetros, tabelas e regras normativas) gera equívocos tanto no cliente quanto na análise interna da concessionária.
...
A norma da Neoenergia que rege esse cálculo é a **DIS-NOR-053**, localizada depois deste levantamento. Metodologia, tabelas e processo de submissão: [`../tecnico/fontes-normativas.md`](../tecnico/fontes-normativas.md).

---

## 2. Método

Cada critério responde a um ponto explícito do desafio.
...
**Critérios descartados:** exigência de cadastro/CREA (a ART é obrigatória nesse tipo de projeto; é requisito legal, não lacuna) e interface para "cliente final" (o usuário real é o projetista que submete e o analista que revisa).

O levantamento gerou evidência para a **questão 13** de [`questões em aberto`](../produto/questoes-em-aberto.md): o portal APR Web da CEMIG é restrito a responsável técnico com CREA. Pelo menos uma concessionária condiciona o acesso ao credenciamento.

---

## 3. Referências Analisadas

Nenhuma outra distribuidora é concorrente direta, pois todas operam em regiões distintas. Elas entram como **referência de processo**.
...
| Referência | Descrição |
|---|---|
| **AltoQi Builder** | Software BIM de projeto elétrico predial, usado antes da submissão. Dimensiona conforme NBR 5410 e normas das concessionárias, gera quadro de cargas e demanda, detecta erros e emite memorial automático. Não se comunica com a concessionária. |

---

## 4. Comparativo

Legenda: `✓` atende; `✕` não atende; `◐` atende em parte (ver notas).

A coluna **Neoenergia** é o estado atual do cliente, levantado em fontes públicas ([`../tecnico/fontes-normativas.md`](../tecnico/fontes-normativas.md)). Não é concorrente: está na tabela para localizar a lacuna no processo que o projeto pretende melhorar.
...
A GED 4621 **define** o modelo de cálculo para medição agrupada com todo o detalhe normativo, mas a CPFL não oferece nenhuma ferramenta que **execute** esse cálculo. A norma é clara, mas a aplicação é 100% manual pelo projetista. Atende no critério normativo, não no de automação.

**2. CPFL, padroniza o resultado `◐`**
Consequência da nota 1: por ser a norma mais explícita do grupo, a GED 4621 reduz a margem de interpretação e tende a produzir resultados mais consistentes entre projetistas. Sem execução automatizada, dois profissionais ainda podem chegar a números diferentes para o mesmo caso, que é o problema do desafio.

**3. Enel, mostra memória de cálculo `◐`**
O simulador entrega o resultado da demanda de forma imediata, mas apresenta o valor final sem o detalhamento das parcelas e fatores aplicados. Serve para conferir a ordem de grandeza. Não substitui o memorial que a concessionária exige anexado ao projeto.
...
**Nenhuma referência junta cálculo e aprovação.** As capacidades são complementares e nunca se sobrepõem:
...
- **Validação prévia é a linha mais reveladora.** Nenhuma concessionária verifica o cálculo antes do envio. A Neoenergia informa os motivos da reprovação e as providências corretivas, mas só depois da análise, que leva até 30 dias. O erro nasce no cálculo manual e só é descoberto no fim do ciclo.

### Implicação para a Solução

O diferencial não é mais uma calculadora; já existem boas. É ocupar o quadrante vazio: cálculo automatizado e padronizado, com **validação antes do envio**, dentro do fluxo oficial de submissão, atendendo o projetista e o analista.

O ganho está em antecipar para a elaboração o que hoje só chega no fim de um ciclo de até 30 dias.

### A Lacuna: Ninguém Junta Cálculo e Processo

Os nove critérios se dividem em dois blocos. Cada grupo de ferramenta falha no bloco que o outro cobre.
...
[`objetivos-projeto.md`](../produto/objetivos-projeto.md) define esses dois blocos como objetivo do MVP, e as propostas são avaliadas contra eles em [`ideacao.md`](../produto/ideacao.md#comparativo-das-propostas).
...
- As normas técnicas de cada distribuidora são públicas e estão referenciadas nas fontes. Os **recursos dos portais** (o que a interface faz ou deixa de fazer) vêm de levantamento próprio da equipe, pois as distribuidoras não publicam documentação funcional dos sistemas.
- Células marcadas com `◐` indicam recurso parcial ou não verificável em fonte pública.
- A GED 4621 da CPFL limita a medição agrupada a 3 a 12 UCs e 75 kVA; acima disso valem outras normas. Recortes equivalentes existem nas demais distribuidoras, o que reforça a ausência de um método único.
...
- [ANEEL: Resolução Normativa nº 1.000/2021](https://www2.aneel.gov.br/cedoc/ren20211000.pdf): regras de prestação do serviço de distribuição (conexão, aprovação de projeto, prazos e responsabilidades). Todas as distribuidoras abaixo precisam observá-la.
...
- [Manual de Apoio do Sistema PEW (PDF)](https://www.copel.com/pewweb/ajuda/ManualApoioPEW.pdf): confirma o fluxo. O projetista cadastrado envia o projeto pronto; o sistema mantém histórico, mas não calcula
