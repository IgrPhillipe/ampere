...
Especificação dos parâmetros recebidos e das saídas produzidas pelo sistema, derivada da DIS-NOR-053 REV 06, da DIS-NOR-030 REV 07 e do protótipo LO-FI.
...
**Todas as demandas são expressas em kVA.** Não há conversão final de kW para kVA: cada parcela já sai em kVA, com os fatores de potência que a DIS-NOR-030 define por parcela.

```
Ded = Drf + Ds + Dc + Dve   [kVA]
```

| Componente | Definição                                       | Método                                       |
| :--------- | :---------------------------------------------- | :------------------------------------------- |
...
A norma usa três notações para a mesma grandeza, e o motor reconhece as três ao conferir contra os exemplos: `Ded` na metodologia (Anexo I, p. 106), `Dte` no agrupamento de torres ou blocos (Anexo I, item 9) e `Df = (Dr × Fr) + Ds` nos exemplos resolvidos, que tratam do caso sem cargas comerciais nem recarga. O nome canônico no sistema é `Ded`.
...
**Os valores das tabelas normativas não são extraídos de PDF nem embutidos no código. São cadastrados manualmente em uma área administrativa do sistema.**
...
- **Dupla leitura obrigatória.** Uma revisão só sai de rascunho após conferência por pessoa diferente de quem cadastrou.
- **Os cinco exemplos do Anexo I são o portão de publicação.** Antes de publicar uma revisão, o sistema executa os cinco exemplos resolvidos da norma com os parâmetros recém-cadastrados. Se algum deixar de fechar, a revisão não é publicada, o que barra erro de digitação nos parâmetros.
- **Revisão publicada não é editada.** Norma nova gera revisão nova, e a anterior passa a substituída. Cálculo antigo continua apontando para a revisão sob a qual foi feito.
...
> A DIS-NOR-053 teve sete revisões em menos de quatro anos, e a DIS-NOR-030 passou da REV 06 para a REV 07 em cinco meses. As duas revisam de forma independente, e **cada cálculo registra as duas revisões aplicadas**.
...
**Parâmetros técnicos.** Determinam o método e as tabelas aplicáveis. O sistema deriva a regra; o projetista não escolhe a tabela.

| Parâmetro                  | Motivo                                                           | Fonte                          |
| :------------------------- | :--------------------------------------------------------------- | :----------------------------- |
...
> Além da demanda da edificação, o **item 6.22.3** exige um segundo cálculo: a demanda individual de cada apartamento pelo método da carga instalada, usada para definir a categoria de fornecimento da unidade. São dois cálculos distintos sobre o mesmo prédio.

---

### 3. Grupos Residenciais: Método da Área Útil (para Calcular Drf)

Dados inseridos por **grupo de unidades**, que reúne unidades de mesma área. A importação por planilha `.xlsx` é alternativa ao preenchimento manual.
...
- **`Fc` é da edificação inteira, não do grupo** (Anexo I, item 3). O agrupamento por área útil serve apenas para obter a demanda unitária de cada tipo. O fator de coincidência vem do total de apartamentos residenciais do prédio.
- **`Fr` é recomendado, não fixo** (Anexo I, item 5). O Quadro 37 traz o valor recomendado por faixa de `Dr`, e a norma admite fator maior "desde que apresentada uma justificativa". É saída derivada por padrão e **entrada opcional acompanhada de justificativa**, registrada no memorial.
...
> **Fonte:** DIS-NOR-030 REV 07, itens 6.27.1 a 6.27.9, p. 47 a 50

O sistema aceita entradas para **todas as nove parcelas** presentes na edificação, não apenas iluminação, TUG, motores e equipamentos especiais.

**A tecnologia da lâmpada é entrada obrigatória** quando a ocupação não é residencial: ela define o fator de potência da parcela `a`, e a diferença entre 0,95 e 0,80 é material no resultado.

**Motores acima de 5 CV e equipamentos especiais** (raios-X, solda a transformador, fornos de indução, eletrólise) geram alerta de validação em tempo real: o sistema solicita o dado adicional necessário em vez de expor um campo para cada caso no formulário principal.
...
**Tabela de fator de demanda aplicável.** As duas normas trazem tabelas diferentes, e a 053 resolve a escolha no Anexo I, item 13: independentemente da potência da estação e de ela estar ou não na medição individual, o `Dve` do quadro geral e do transformador usa o fator do item 6.26 da 053.
...
> **Parcela `j`.** A DIS-NOR-030 define a fórmula com nove parcelas (`a` a `i`) e, no item 6.27.10, uma décima parcela `j` para estação de recarga, que não entra no somatório. Na 053 a recarga é termo próprio (`Dve`), e o motor trata `j` fora da soma das nove. Somar as duas conta a recarga duas vezes.
...
Verificações executadas pelo motor e reportadas junto com o resultado:
...
A DIS-NOR-053 teve sete revisões em menos de quatro anos, e a DIS-NOR-030 revisa em ritmo próprio. Dois cálculos idênticos feitos sob revisões diferentes podem produzir resultados diferentes. Cada execução do motor gera um registro persistido.
...
| `minimumApplied`                                  | Booleano: indica se o mínimo normativo sobrepôs o cálculo                              |
...
Pontos em que as normas são omissas e o motor da US04 adotou um critério. Cada um aparece na memória de cálculo, para permitir contestação por quem confere.
...
| 11 | Cada parcela é arredondada em duas casas antes da soma | A soma exibida fecha com os números mostrados |
...
| Fator de potência de tomadas: 1,00 na 030 contra 0,80 no Exemplo 1 da 053 | Vale a 030, que o Anexo I, item 7.2, manda usar                                      | O motor não reproduz a parcela `a` do Exemplo 1 na casa dos centavos: 4,50 contra 4,69 kVA. O resultado final daquele exemplo não muda, porque o mínimo por tensão prevalece |
...
| :-- | :------------------------------------------------------------------------ | :----------------------------------------------- | :------------------ | :------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | DIS-NOR-053: Edificações com Múltiplas Unidades Consumidoras até 34,5 kV  | Neoenergia (Coelba, Pernambuco, Cosern, Elektro) | REV 06, 09/09/2025  | Metodologia e exemplos: Anexo I, p. 106 a 133. Tipos de edificação: 6.22 a 6.25. Recarga veicular: 6.26. Projeto elétrico e documentação: 6.27 |
| 2   | DIS-NOR-030: Tensão Secundária a Edificações Individuais                  | Neoenergia Pernambuco                            | REV 07, 17/04/2026  | Carga instalada: item 6.27, p. 47 a 50. Recarga veicular: 6.26.4. Tabelas de fator de demanda: 6, 7, 8, 9, 11 a 16, 18 e 22                  |
